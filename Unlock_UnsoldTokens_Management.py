...
ax3.set_ylim(bottom=-0.05, top=max(overflow) * 1.2)
...
reward_usd = np.array(allocations["Rewards"]["unlock_schedule"]) * max_supply * median_path
ax.bar(range(40), reward_usd, bottom=bottom, color=allocations["Rewards"]["color"], label="Rewards", alpha=0.7)
bottom += reward_usd


# Overflow Hatching for Cumulative Unlocks
cumulative_excess = np.maximum(np.maximum(lower_unlocks_usd, median_unlocks_usd), upper_unlocks_usd) - dynamic_market_depth
ax5.bar(
    range(40),
